...
public void symulacjaUkladu (double symulacja){
	//tworzymy obiekty klasy Vecotr2D dla poszczególnych si³. Wykorzystujemy klase Vector2D w obliczeniach
	 ...
	 FSpr=pPunktuZawieszenia.wektorNorm().mnozenieWektora(wspSp*(pMasy.roznicaWektorow(pPunktuZawieszenia).dlugoscWektora()-lSwobodna)); //obliczamy si³e sprê¿yatoœci 
	 FWyp=FSpr.sumaWektorow(Fb).sumaWektorow(FGraw); //obliczamy si³ê wypadkow¹
	 Fb=vMasy.mnozenieWektora(wspTl); //obliczamy t³umienie
	 vMasy=vMasy.sumaWektorow(FWyp.mnozenieWektora(symulacja/masa)); //obliczamy prêdkosc	
	 pMasy=pMasy.sumaWektorow(vMasy.mnozenieWektora(symulacja/masa)); //oraz po³o¿enie masy
}
